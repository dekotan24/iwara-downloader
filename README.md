...
- **UUIDベース重複検出** (v1.1.1〜) - mp4ファイルに iwara のUUIDを埋め込み、ファイル名を変更してもDBが消えても重複DLを防止
...
> ⚠️ **v1.1.1以降ログインが必須になりました。** 未ログイン状態ではダウンロード機能が利用できません（iwara 本家の挙動に準拠）。トークンの有効期限は起動時に自動検証され、期限切れ時は自動ログアウトされます。
...
| デフォルト画質 | Source / 540p / 360p（iwaraが配信する3画質） | Source |
...
├── x_version_secret.txt      # iwara X-Versionシークレット（v1.1.1〜、30日キャッシュ）
└── logs/                     # ログファイル
    └── IwaraDownloader_YYYYMMDD_HHMMSS.log
```

また、各アーティストフォルダ内には隠しファイルとして `.iwara_index.json` が作成されます（v1.1.1〜、UUID照合スキャンの高速化用）。
...
- ログイン状態を確認してください（v1.1.1〜ログイン必須）
- 動画が非公開または削除されていないか確認してください
- ディスク容量を確認してください
- 403エラーが頻発する場合はレート制限の値を大きくしてください

### 全動画が360pでしかダウンロードできない（v1.1.0以前の不具合）

v1.1.0以前では iwara のフロントエンド更新により X-Version シークレットが変わり、filesq APIから 360p/preview しか取得できなくなる不具合がありました。v1.1.1で恒久対策済みです：
- 埋め込みシークレット → キャッシュ → 動的取得（main.jsから抽出）の3段階
- 低画質フォールバックを検出した時のみ `main.js` を取得して新しいシークレットを抽出し、30日キャッシュ

### 既存ファイルをUUID対応にしたい（v1.1.1〜）

設定画面 → **バックアップ**タブ → 「既存ファイルにタグを書き込む」ボタンで、DB登録済みの全mp4ファイルに iwara のUUIDタグを一括で書き込めます。処理中に設定画面を閉じようとすると確認ダイアログが出ます（中断しても再実行で続きから処理されます）。
...
- NAudio - 音声再生
- TagLibSharp - mp4コンテナへのUUIDメタデータ書き込み（v1.1.1〜）
